import {
  Lucid,
  Blockfrost,
  Constr,
  Data,
} from "https://unpkg.com/lucid-cardano@0.10.11/web/mod.js";

/* =====================================================
   DATUM SCHEMA (MUST MATCH PLUTUS)
===================================================== */

const PricePredictionDatum = Data.Object({
  ppPredictor: Data.Bytes(),
  ppTargetEpoch: Data.Integer(),
  ppPredictedPrice: Data.Integer(),
  ppStake: Data.Integer(),
  ppOracle: Data.Bytes(),
  ppResolved: Data.Boolean(),
});

/* =====================================================
   REDEEMERS
===================================================== */

const resolveRedeemer = Data.to(new Constr(1, []));
const claimRedeemer   = Data.to(new Constr(2, []));

/* =====================================================
   CONFIG
===================================================== */

const BLOCKFROST_URL =
  "https://cardano-preprod.blockfrost.io/api/v0";
const BLOCKFROST_KEY =
  "preprodYjRkHfcazNkL0xxG9C2RdUbUoTrG7wip";
const NETWORK = "Preprod";

/* =====================================================
   PLUTUS SCRIPT
===================================================== */

const SCRIPT_CBOR = "";

const script = { type: "PlutusV2", script: SCRIPT_CBOR };

/* =====================================================
   GLOBAL STATE
===================================================== */

let lucid;
let walletAddress;
let scriptAddress;
let currentEpoch = null;
let userStats = {
  totalPredictions: 0,
  totalStaked: 0n,
  wins: 0,
  accuracy: 0
};

/* =====================================================
   UI LOGGING - ENHANCED
===================================================== */

function log(message, type = "info") {
  const logElement = document.getElementById("log");
  const timestamp = new Date().toLocaleTimeString();
  
  let color = "";
  let prefix = "";
  
  switch(type) {
    case "success": 
      color = "color: #86efac;"; 
      prefix = "✅ ";
      break;
    case "error": 
      color = "color: #fca5a5;"; 
      prefix = "❌ ";
      break;
    case "warning": 
      color = "color: #fcd34d;"; 
      prefix = "⚠️ ";
      break;
    default: 
      color = "color: #93c5fd;"; 
      prefix = "ℹ️ ";
  }
  
  const logEntry = document.createElement("div");
  logEntry.style.cssText = "margin-bottom: 4px; padding-bottom: 4px; border-bottom: 1px solid #1e293b;";
  logEntry.innerHTML = `<span style="color: #64748b;">[${timestamp}]</span> <span style="${color}">${prefix}${message}</span>`;
  
  logElement.prepend(logEntry);
  
  // Keep only last 15 entries
  if (logElement.children.length > 15) {
    logElement.children[logElement.children.length - 1].remove();
  }
}

/* =====================================================
   NOTIFICATION SYSTEM
===================================================== */

function showNotification(message, type = "info") {
  const notification = document.createElement("div");
  notification.style.cssText = `
    position: fixed;
    top: 20px;
    right: 20px;
    padding: 16px 24px;
    background: ${type === 'success' ? 'rgba(34, 197, 94, 0.95)' : 
                type === 'error' ? 'rgba(239, 68, 68, 0.95)' : 
                'rgba(56, 189, 248, 0.95)'};
    color: white;
    border-radius: 12px;
    font-weight: 600;
    z-index: 9999;
    backdrop-filter: blur(8px);
    animation: slideIn 0.3s ease;
    box-shadow: 0 10px 30px rgba(0,0,0,0.3);
  `;
  
  notification.innerHTML = `
    <div style="display: flex; align-items: center; gap: 12px;">
      <i class="fas fa-${type === 'success' ? 'check-circle' : 
                         type === 'error' ? 'exclamation-circle' : 
                         'info-circle'}"></i>
      <span>${message}</span>
    </div>
  `;
  
  document.body.appendChild(notification);
  
  setTimeout(() => {
    notification.style.animation = 'slideOut 0.3s ease';
    setTimeout(() => notification.remove(), 300);
  }, 5000);
}

// Add animation styles
const style = document.createElement('style');
style.textContent = `
  @keyframes slideIn {
    from { transform: translateX(100%); opacity: 0; }
    to { transform: translateX(0); opacity: 1; }
  }
  @keyframes slideOut {
    from { transform: translateX(0); opacity: 1; }
    to { transform: translateX(100%); opacity: 0; }
  }
`;
document.head.appendChild(style);

/* =====================================================
   FETCH CURRENT EPOCH
===================================================== */

async function fetchCurrentEpoch() {
  try {
    const response = await fetch(
      `${BLOCKFROST_URL}/epochs/latest`,
      { headers: { 'project_id': BLOCKFROST_KEY } }
    );
    
    if (response.ok) {
      const data = await response.json();
      currentEpoch = data.epoch;
      
      // Update UI if elements exist
      const epochInput = document.getElementById("epoch");
      if (epochInput) {
        epochInput.value = currentEpoch + 2;
        epochInput.min = currentEpoch + 1;
      }
      
      log(`Current epoch: ${currentEpoch}`, "info");
      return currentEpoch;
    }
  } catch (error) {
    log(`Could not fetch epoch: ${error.message}`, "warning");
  }
  return null;
}

/* =====================================================
   SHORTEN ADDRESS
===================================================== */

function shortenAddress(addr, chars = 6) {
  if (!addr) return "";
  return `${addr.substring(0, chars)}...${addr.substring(addr.length - chars)}`;
}

/* =====================================================
   LOAD USER STATS
===================================================== */

async function loadUserStats() {
  if (!lucid || !walletAddress) return;
  
  try {
    const predictorPkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);
    
    let totalStaked = 0n;
    let predictions = 0;
    let wins = 0;
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        if (datum.ppPredictor === predictorPkh) {
          predictions++;
          totalStaked += datum.ppStake;
          
          if (datum.ppResolved) {
            wins++;
          }
        }
      } catch (e) {
        continue;
      }
    }
    
    userStats = {
      totalPredictions: predictions,
      totalStaked: totalStaked,
      wins: wins,
      accuracy: predictions > 0 ? Math.round((wins / predictions) * 100) : 0
    };
    
    // Update UI if elements exist
    updateUserUI();
    
  } catch (error) {
    console.error("Error loading user stats:", error);
  }
}

/* =====================================================
   UPDATE USER UI
===================================================== */

function updateUserUI() {
  // Update portfolio if elements exist
  const userStaked = document.getElementById("userStaked");
  const userWins = document.getElementById("userWins");
  const userAccuracy = document.getElementById("userAccuracy");
  
  if (userStaked) {
    userStaked.innerHTML = `₳ ${(Number(userStats.totalStaked) / 1000000).toLocaleString()}`;
  }
  
  if (userWins) {
    userWins.innerHTML = userStats.wins;
  }
  
  if (userAccuracy) {
    userAccuracy.innerHTML = `${userStats.accuracy}%`;
  }
  
  // Update predictions list
  displayUserPredictions();
}

/* =====================================================
   DISPLAY USER PREDICTIONS
===================================================== */

async function displayUserPredictions() {
  const predictionsList = document.getElementById("predictionsList");
  if (!predictionsList || !lucid || !walletAddress) return;
  
  try {
    const predictorPkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);
    
    let html = '';
    let count = 0;
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        if (datum.ppPredictor === predictorPkh && count < 5) {
          const status = datum.ppResolved ? '✅' : '⏳';
          const priceAda = (Number(datum.ppPredictedPrice) / 1000000).toFixed(3);
          const stakeAda = (Number(datum.ppStake) / 1000000).toFixed(0);
          
          html += `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid #1e293b;">
              <div>
                <span style="color: #38bdf8; font-weight: 600;">Epoch ${datum.ppTargetEpoch}</span>
                <div style="font-size: 0.8rem; color: #94a3b8;">₳${priceAda} | Stake: ₳${stakeAda}</div>
              </div>
              <span style="color: ${datum.ppResolved ? '#86efac' : '#fcd34d'};">${status}</span>
            </div>
          `;
          count++;
        }
      } catch (e) {
        continue;
      }
    }
    
    if (count === 0) {
      html = '<div style="color: #94a3b8; text-align: center; padding: 16px;">No predictions yet</div>';
    }
    
    predictionsList.innerHTML = html;
    
  } catch (error) {
    console.error("Error displaying predictions:", error);
  }
}

/* =====================================================
   LOAD LEADERBOARD
===================================================== */

async function loadLeaderboard() {
  const leaderboardEl = document.getElementById("leaderboard");
  if (!leaderboardEl || !lucid) return;
  
  try {
    const utxos = await lucid.utxosAt(scriptAddress);
    const stats = new Map();
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        const predictor = datum.ppPredictor;
        
        if (!stats.has(predictor)) {
          stats.set(predictor, { wins: 0, total: 0, staked: 0n });
        }
        
        const s = stats.get(predictor);
        s.total++;
        s.staked += datum.ppStake;
        if (datum.ppResolved) s.wins++;
        
      } catch (e) {
        continue;
      }
    }
    
    // Sort by wins
    const leaderboard = Array.from(stats.entries())
      .map(([addr, data]) => ({
        address: addr,
        wins: data.wins,
        accuracy: data.total > 0 ? Math.round((data.wins / data.total) * 100) : 0,
        staked: data.staked
      }))
      .sort((a, b) => b.wins - a.wins || Number(b.staked - a.staked))
      .slice(0, 10);
    
    let html = '';
    leaderboard.forEach((item, index) => {
      const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}.`;
      html += `
        <div style="display: flex; align-items: center; gap: 12px; padding: 8px; border-bottom: 1px solid #1e293b;">
          <div style="width: 30px; height: 30px; background: rgba(56,189,248,0.1); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 700;">
            ${medal}
          </div>
          <div style="flex: 1;">
            <div style="font-family: monospace; color: white;">${shortenAddress(item.address, 4)}</div>
            <div style="font-size: 0.75rem; color: #94a3b8;">${item.wins} wins • ${item.accuracy}% acc</div>
          </div>
          <div style="font-weight: 600; color: #38bdf8;">₳${(Number(item.staked) / 1000000).toFixed(0)}</div>
        </div>
      `;
    });
    
    leaderboardEl.innerHTML = html;
    
  } catch (error) {
    console.error("Error loading leaderboard:", error);
  }
}

/* =====================================================
   LOAD GLOBAL STATS
===================================================== */

async function loadGlobalStats() {
  if (!lucid) return;
  
  try {
    const utxos = await lucid.utxosAt(scriptAddress);
    
    let totalStaked = 0n;
    let activePredictions = 0;
    let uniqueUsers = new Set();
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        totalStaked += datum.ppStake;
        uniqueUsers.add(datum.ppPredictor);
        
        if (!datum.ppResolved) {
          activePredictions++;
        }
      } catch (e) {
        continue;
      }
    }
    
    // Update UI if elements exist
    const totalTVL = document.getElementById("totalTVL");
    const activePredictionsEl = document.getElementById("activePredictions");
    const totalStakedEl = document.getElementById("totalStaked");
    const uniqueUsersEl = document.getElementById("uniqueUsers");
    
    if (totalTVL) totalTVL.innerHTML = `₳ ${(Number(totalStaked) / 1000000).toLocaleString()}`;
    if (activePredictionsEl) activePredictionsEl.innerHTML = activePredictions;
    if (totalStakedEl) totalStakedEl.innerHTML = `₳ ${(Number(totalStaked) / 1000000).toLocaleString()}`;
    if (uniqueUsersEl) uniqueUsersEl.innerHTML = uniqueUsers.size;
    
  } catch (error) {
    console.error("Error loading global stats:", error);
  }
}

/* =====================================================
   INIT / CONNECT - ENHANCED
===================================================== */

async function init() {
  if (!window.cardano?.lace) {
    showNotification("Lace wallet not found. Please install it first.", "error");
    return;
  }

  try {
    lucid = await Lucid.new(
      new Blockfrost(BLOCKFROST_URL, BLOCKFROST_KEY),
      NETWORK
    );

    const api = await window.cardano.lace.enable();
    lucid.selectWallet(api);

    walletAddress = await lucid.wallet.address();
    scriptAddress = lucid.utils.validatorToAddress(script);

    const balance = await lucid.wallet.getBalance();
    const collateral = await lucid.wallet.getCollateral();

    // Update wallet badge
    const walletBadge = document.getElementById("walletBadge");
    if (walletBadge) {
      walletBadge.innerHTML = `
        <i class="fas fa-wallet"></i>
        <span>${shortenAddress(walletAddress)}</span>
        <span style="background: rgba(34,197,94,0.2); padding: 2px 8px; border-radius: 40px; margin-left: 8px; color: #86efac;">
          <i class="fas fa-circle" style="font-size: 0.5rem;"></i> Connected
        </span>
      `;
    }

    // Update connect/disconnect UI
    const disconnectedContent = document.getElementById("disconnectedContent");
    const connectedContent = document.getElementById("connectedContent");
    
    if (disconnectedContent) disconnectedContent.style.display = "none";
    if (connectedContent) connectedContent.style.display = "block";

    // Load data
    await fetchCurrentEpoch();
    await loadUserStats();
    await loadGlobalStats();
    await loadLeaderboard();

    // Check collateral
    if (collateral.length === 0) {
      showNotification("Please set collateral in your wallet", "warning");
      log("WARNING: No collateral set - you won't be able to submit transactions", "warning");
    }

    log(`Connected: ${shortenAddress(walletAddress)}`, "success");
    log(`Balance: ${Number(balance.lovelace / 1_000_000n)} ADA`, "info");
    showNotification("Wallet connected successfully!", "success");

  } catch (error) {
    log(`Connection failed: ${error.message}`, "error");
    showNotification(error.message, "error");
    console.error(error);
  }
}

/* =====================================================
   SUBMIT PREDICTION - ENHANCED
===================================================== */

async function submitPrediction() {
  if (!lucid || !walletAddress) {
    showNotification("Please connect wallet first", "error");
    return;
  }

  try {
    const epoch = BigInt(document.getElementById("epoch").value);
    const price = BigInt(document.getElementById("price").value);
    const stakeAda = BigInt(document.getElementById("stake").value) * 1_000_000n;
    const oracleAddr = document.getElementById("oracle").value;

    // Validation
    if (currentEpoch && epoch <= BigInt(currentEpoch)) {
      throw new Error(`Target epoch must be greater than current epoch (${currentEpoch})`);
    }
    
    if (stakeAda < 10_000_000n) {
      throw new Error("Minimum stake is 10 ADA");
    }

    if (!oracleAddr.startsWith("addr_test")) {
      throw new Error("Please enter a valid testnet address");
    }

    const predictorPkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const oraclePkh = lucid.utils.getAddressDetails(oracleAddr).paymentCredential.hash;

    const datum = mkPredictionDatum(
      predictorPkh,
      oraclePkh,
      epoch,
      price,
      stakeAda
    );

    log(`Building transaction for ${stakeAda / 1_000_000n} ADA...`, "info");

    const tx = await lucid
      .newTx()
      .payToContract(
        scriptAddress,
        { inline: datum },
        { lovelace: stakeAda + 2_000_000n }
      )
      .addSigner(walletAddress)
      .complete();

    const signed = await tx.sign().complete();
    const txHash = await signed.submit();

    // Success
    log(`✅ Prediction submitted! TX: ${txHash}`, "success");
    showNotification(`Prediction submitted! Epoch ${epoch}`, "success");
    
    // Clear stake field
    document.getElementById("stake").value = "";
    
    // Refresh stats
    await loadUserStats();
    await loadGlobalStats();
    await loadLeaderboard();

  } catch (e) {
    log(`❌ ${e.message}`, "error");
    showNotification(e.message, "error");
  }
}

/* =====================================================
   DATUM BUILDER
===================================================== */

function mkPredictionDatum(
  predictorPkh,
  oraclePkh,
  epoch,
  price,
  stake
) {
  return Data.to(
    {
      ppPredictor: predictorPkh,
      ppTargetEpoch: epoch,
      ppPredictedPrice: price,
      ppStake: stake,
      ppOracle: oraclePkh,
      ppResolved: false,
    },
    PricePredictionDatum
  );
}

/* =====================================================
   RESOLVE PREDICTION (ORACLE) - ENHANCED
===================================================== */

async function resolvePrediction() {
  if (!lucid || !walletAddress) {
    showNotification("Please connect wallet first", "error");
    return;
  }

  try {
    const oraclePkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);

    const predictionToResolve = utxos.find((u) => {
      if (!u.datum) return false;
      try {
        const d = Data.from(u.datum, PricePredictionDatum);
        return d.ppOracle === oraclePkh && !d.ppResolved;
      } catch {
        return false;
      }
    });

    if (!predictionToResolve) {
      log("No unresolved predictions found for your oracle address", "info");
      return;
    }

    const oldDatum = Data.from(predictionToResolve.datum, PricePredictionDatum);
    const newDatum = Data.to(
      { ...oldDatum, ppResolved: true },
      PricePredictionDatum
    );

    const tx = await lucid
      .newTx()
      .collectFrom([predictionToResolve], resolveRedeemer)
      .attachSpendingValidator(script)
      .payToContract(
        scriptAddress,
        { inline: newDatum },
        predictionToResolve.assets
      )
      .addSigner(walletAddress)
      .complete();

    const signed = await tx.sign().complete();
    const txHash = await signed.submit();

    log(`✅ Prediction resolved! TX: ${txHash}`, "success");
    showNotification("Prediction resolved successfully", "success");
    
    // Refresh data
    await loadUserStats();
    await loadGlobalStats();
    await loadLeaderboard();

  } catch (e) {
    log(`❌ ${e.message}`, "error");
    showNotification(e.message, "error");
  }
}

/* =====================================================
   CLAIM REWARD - ENHANCED
===================================================== */

async function claimReward() {
  if (!lucid || !walletAddress) {
    showNotification("Please connect wallet first", "error");
    return;
  }

  try {
    const predictorPkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);

    const claimableUtxo = utxos.find((u) => {
      if (!u.datum) return false;
      try {
        const d = Data.from(u.datum, PricePredictionDatum);
        return d.ppPredictor === predictorPkh && d.ppResolved;
      } catch {
        return false;
      }
    });

    if (!claimableUtxo) {
      log("No claimable rewards found", "info");
      return;
    }

    const datum = Data.from(claimableUtxo.datum, PricePredictionDatum);

    const tx = await lucid
      .newTx()
      .collectFrom([claimableUtxo], claimRedeemer)
      .attachSpendingValidator(script)
      .payToAddress(walletAddress, {
        lovelace: datum.ppStake,
      })
      .addSigner(walletAddress)
      .complete();

    const signed = await tx.sign().complete();
    const txHash = await signed.submit();

    const rewardAda = datum.ppStake / 1_000_000n;
    log(`💰 Reward claimed: ${rewardAda} ADA! TX: ${txHash}`, "success");
    showNotification(`Reward claimed: ${rewardAda} ADA`, "success");
    
    // Refresh stats
    await loadUserStats();
    await loadGlobalStats();

  } catch (e) {
    log(`❌ ${e.message}`, "error");
    showNotification(e.message, "error");
  }
}

/* =====================================================
   CHECK PREDICTIONS
===================================================== */

async function checkMyPredictions() {
  if (!lucid || !walletAddress) {
    showNotification("Please connect wallet first", "error");
    return;
  }

  try {
    const predictorPkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);
    
    let count = 0;
    let pending = 0;
    let resolved = 0;
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        if (datum.ppPredictor === predictorPkh) {
          count++;
          if (datum.ppResolved) resolved++;
          else pending++;
        }
      } catch (e) {
        continue;
      }
    }
    
    log(`📊 Your predictions: ${count} total (${pending} pending, ${resolved} resolved)`, "info");
    
  } catch (e) {
    log(`❌ ${e.message}`, "error");
  }
}

/* =====================================================
   CHECK UNRESOLVED (ORACLE)
===================================================== */

async function checkUnresolvedPredictions() {
  if (!lucid || !walletAddress) {
    showNotification("Please connect wallet first", "error");
    return;
  }

  try {
    const oraclePkh = lucid.utils.getAddressDetails(walletAddress).paymentCredential.hash;
    const utxos = await lucid.utxosAt(scriptAddress);
    
    let count = 0;
    
    for (const utxo of utxos) {
      if (!utxo.datum) continue;
      
      try {
        const datum = Data.from(utxo.datum, PricePredictionDatum);
        if (datum.ppOracle === oraclePkh && !datum.ppResolved) {
          count++;
        }
      } catch (e) {
        continue;
      }
    }
    
    log(`🔍 You have ${count} unresolved prediction(s) to resolve`, count > 0 ? "warning" : "info");
    
  } catch (e) {
    log(`❌ ${e.message}`, "error");
  }
}

/* =====================================================
   REFRESH ALL DATA
===================================================== */

async function refreshAllData() {
  if (!lucid) return;
  
  log("🔄 Refreshing data...", "info");
  
  await Promise.all([
    fetchCurrentEpoch(),
    loadUserStats(),
    loadGlobalStats(),
    loadLeaderboard()
  ]);
  
  log("✅ Data refreshed", "success");
}

/* =====================================================
   COPY REFERRAL LINK
===================================================== */

function copyReferralLink() {
  if (!walletAddress) {
    showNotification("Connect wallet to generate referral link", "warning");
    return;
  }
  
  const link = `https://cardanopredict.app/ref/${btoa(walletAddress)}`;
  navigator.clipboard.writeText(link);
  showNotification("Referral link copied!", "success");
}

/* =====================================================
   EVENT LISTENERS
===================================================== */

document.addEventListener("DOMContentLoaded", () => {
  // Set default values
  const oracleInput = document.getElementById("oracle");
  if (oracleInput) {
    oracleInput.value = "addr_test1qzx9hu8j4ah3auytk0mwcupd69hpc52t0cw39a65ndrah86djs784u92a3m5w475w3w35tyd6v3qumkze80j8a6h5tuqq5xe8y";
  }
  
  const stakeInput = document.getElementById("stake");
  if (stakeInput) {
    stakeInput.value = "10";
  }
  
  // Fetch epoch on load
  fetchCurrentEpoch();
  
  // Set up event listeners
  document.getElementById("connect")?.addEventListener("click", init);
  document.getElementById("submit")?.addEventListener("click", submitPrediction);
  document.getElementById("resolve")?.addEventListener("click", resolvePrediction);
  document.getElementById("claim")?.addEventListener("click", claimReward);
  document.getElementById("fetchCurrent")?.addEventListener("click", fetchCurrentEpoch);
  document.getElementById("checkPredictions")?.addEventListener("click", checkMyPredictions);
  document.getElementById("checkUnresolved")?.addEventListener("click", checkUnresolvedPredictions);
  document.getElementById("refreshData")?.addEventListener("click", refreshAllData);
  document.getElementById("clearLog")?.addEventListener("click", () => {
    const logEl = document.getElementById("log");
    if (logEl) logEl.innerHTML = "";
    log("Log cleared", "info");
  });
  
  log("🚀 Cardano Predict dApp loaded", "success");
  log("📊 Connect your Lace wallet to start", "info");
});

/* =====================================================
   EXPORT FOR DEBUGGING
===================================================== */

window.app = {
  init,
  submitPrediction,
  resolvePrediction,
  claimReward,
  checkMyPredictions,
  checkUnresolvedPredictions,
  refreshAllData,
  copyReferralLink,
  log,
  shortenAddress
};
